 ...
 * в начале файла).
 *  - Команды Doxygen напоминают команды LaTeX, но для аргументов команд не нужны дополнительные
 * символы. Команда может начинаться с символа \p \\, либо \p \\@. Какой символ использовать - дело вкуса, однако,
 * мы рекомендуем c символа \p \\@ начинать команды секционирования, а в пределах секции использовать \p \\ в пользу
 * наглядности. Разделителем для аргументов является пробел. У аргументов есть область действия, которая
 * указана в документации Doxygen следующим образом:
 *    - если в документации аргумент записан в скобках \< \>, то аргументом является одно слово до следующего пробела;
 *    - если в круглых скобках \( \), то аргумент действует до конца текущей строки или до следующей команды;
 *    - если в фигурных скобках { }, то до конца параграфа. Напомним, что параграфы разделяются пустыми строками,
 *      либо концом комментация, либо следующей командой секционирования.
 * @version 0.1
 *
 ...
 * 
 * \section h1_sec_3 Копирование уже написанных фрагментов
 * В Doxygen предусмотрены инструменты для копирования уже написанных фрагментов для избежания
 * дублирования материала. Вы можете копировать как все содержимое, так и отдельные фрагменты (краткое или
 * детальное содержимое). Для
 * этого нужно использовать команды \p \\copydoc, \p \\copybrief, \\p \\copydetails.
 * 
 * Например, все нижестоящие фрагменты, очерченные линиями, скопированы.
 * <hr>
 * \copydoc no_std::File
 * <hr>
 * \copybrief lib_read(int ,char*,size_t)
 * <hr>
 * \copydetails lib_read(int ,char*,size_t)
 * 
 * <hr>
 * 
