 ...
 *  
 * В абзаце можно делать переносы строк произвольно с помощью тега \<br\>, либо
 ...
 * делать перекрестные ссылки командой \\ref. Это расширение Doxygen - в стандартном Markdown его нет.
 * 
 * Также якоря можно рассталять в любой части документации с помощью команды \\anchor.
 * 
 ...
 * - Третий элемент
 *
 * Одноуровневые списки можно создавать также командой \\arg.\n
 * \arg Яблоко
 * \arg Апельсин
 * \arg Ананас
 * 
 ...
 * 
 * Для создания блоков с кодом можно использовать блок \\code...\\endcode. Для лучшей
 ...
 * 
 * С помощью команды \\see, можно делать красивые вставки, чтобы ссылаться на некоторый материал, описанный в другом месте.
 ...
 * как имена файлов. Если Doxygen найдет этот файл при сканировании, то он разрешит ссылку \\ref на него. Например, мы
 ...
 * 
 * Кроме того, есть возможность выборочно набирать фрагмент с помощью команд \\include, \\dontinclude, \\until, \\skip,
 * \\line, \\skipline. Рекомендуется такой подход исключительно для кода, оформленного в отдельных файлах, так как
 * маленькие куски из большого файла проще извлекать с помощью \\snippet.
 * 
 * \paragraph h1_sec_sec_sec_sec_3 Вставка изображений
 * Для вставки изображений используется команда \\image. Для документации в формате HTML формат изображений ограничивается
 * используемым браузером. Для LaTeX формат изображений должен поддерживаться используемым компилятором LaTeX. Для HTML
 * и для LaTeX документации формат команды \\image немного отличается.
 * 
...
//! Эта функция будет вставлена в документацю целиком. Мы используем команду \\noop, чтобы текст метки не попал в выдачу.
//! \noop [Пример]
